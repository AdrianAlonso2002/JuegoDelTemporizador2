class Pokemon {
	...
	atacar(atacante, atacado, attack) {
		if (!attack || !attack.target || !attack.target.value) {
			console.error('Error: El objeto de ataque es inválido.');
			return;
		}
		...
		switch (random) {
			...
			case 9:
				...
				break;
			case 10:
				$txtBattle.innerHTML = `<strong>${atacado.name}</strong> evadió el ataque ${attack.target.value} de <strong>${atacante.name}</strong>`;
				evadAtaque = true;
				break;
		}
	}
